...
fileprivate class OrderToPrint {
    ...
    var cartItems: [ReceiptModel]
    
    init(orderDetails: Orderdetail, cartItems: [ReceiptModel]) {
...
class PrintJobHelper {
    ...
    static func setPrinters() {
     ...
     static func getPrinterForCheckoutPrinting() -> PrinterDetailsModel? {
        availablePrinters.removeAll()
        if let printerDetailModels = getPrinterDetailsModels(printerType: .all) {
            for printerDetailModel in printerDetailModels {
                availablePrinters.append(printerDetailModel)
                ...
                case .checkout:
                    printJob.printCheckoutReceipt(printerDetails: printer, orderDetails: orderToPrint.orderDetails, cartItems: orderToPrint.cartItems) { (isSuccess, message) in
                        ...
                        if !isSuccess {
                            ...
                            print("Amjad here")
                            UIUtility.showAlert(title: message)//here
                            print(message)
                        }
                    }
                case .kitchen:
                    print("Pressed for Kitchen")
...
//                    }
    ...
    /// - Returns: Void
    static func addKitchenOrderInPrinterQueue(orderDetails: Orderdetail, cartItems: [ReceiptModel]) {
    ...
    ///     - cartItems: [CartItemStruct]
    /// - Returns: Void
    static func addCheckoutOrderInPrinterQueue(orderDetails: Orderdetail, cartItems: [ReceiptModel]) {
...
fileprivate class PrintJob {
    ...
    func printPackagingReceipt(orderToPrint: Orderdetail, completion: @escaping (Bool, String) -> ()) {
        if let printerDetails = PrintJobHelper.getPrinterDetailsModels(printerType: .checkout)?.first {
            ...
            packagingReceiptBuilder.setWidthOfReceipt(width: CGFloat(Constants._4inchScale))
    ...
    ///     - cartItems: [CartItemStruct]
    ///     - (Bool, String) -> ()
    func printCheckoutReceipt(printerDetails: PrinterDetailsModel, orderDetails: Orderdetail, cartItems: [ReceiptModel], completion: @escaping (Bool, String) -> ()) {
...
//            break
        ...
        checkoutReceiptBuilder.setWidthOfReceipt(width: CGFloat(Constants._4inchScale))
        print(checkoutReceiptBuilder.createReceiptImage())
    ...
    ///     - (Bool, String) -> ()
//    func printKitchenReceipt(printerDetails: PrinterDetailsModel, orderDetails: Orderdetail, cartItems: [CartItemStruct], completion: @escaping (Bool, String) -> ()) {
//
//        switch printerDetails.manufacturer! {
//        case PrinterManufacturer.epson:
//            printerContext = PrinterContext(strategy: EpsonPrinterHelper())
